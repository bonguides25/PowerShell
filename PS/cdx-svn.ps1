...
# Get tenant information
    ...
    Write-Host "Tenant: $($tenantInfo.DisplayName)" -ForegroundColor Yellow
    Write-Host "Id    : $($tenantInfo.Id)" -ForegroundColor Yellow
    Write-Host "Domain: $((Get-MgDomain).Id)" -ForegroundColor Yellow
    Write-Host

# Creating users in bulk
    Write-Host "Creating user accounts..." -ForegroundColor Yellow 
    $domain = Get-MgDomain | Select-Object -ExpandProperty Id
    $items = @(1..6)
...
# Wait for all users have been created    
    # Write-Host "Waiting for accounts being created..." -ForegroundColor Yellow 
...
# Add users to Global Admin role
    Write-Host "Add users to Global Admin role..." -ForegroundColor Yellow
...
# Assign license to users
    ...
    Write-Host "Assign licenses to users..." -ForegroundColor Green
    $users = Get-MgUser -ConsistencyLevel eventual -Count userCount -Filter "startsWith(DisplayName, 'Account')" -OrderBy UserPrincipalName
    Start-Sleep 1
...
# Add users to group sg-CloudPCUsers
    Write-Host "Add users to group sg-CloudPCUsers..." -ForegroundColor Green   
    $i = 1
    foreach ($user in $users) {
